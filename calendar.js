...
var openDescription = function openDescription(num, month, year) {
  ...
  $('#' + month + '_' + year + '_day_' + num).data('flag', 'open');
};

var closeDescription = function closeDescription() {
  $('.calendar_desciption').hide();
  $('.day').data('flag', 'close');
};
...
function datesClickEvent(month, year) {
  var firstDay = zeller(month, year);
  var totalDay = getMonthDays(month, year);

  var _loop = function _loop(i) {
    $('#' + month + '_' + year + '_day_' + i).click(function () {
      console.log('you click' + month + '__' + year + '_' + i);
      // Open selected and close all others
      $('#' + month + '_' + year + '_day_' + i).data('flag') === 'close' ? openDescription(i, month, year) : closeDescription();
    });
  };

  for (var i = firstDay; i < firstDay + totalDay; i++) {
    _loop(i);
  }
}
      ...
      if (i !== 1 && i !== 2 && i !== 10 && i !== 18 && i !== 26 && i !== 34 && i !== 42 && j > 0) {
        ...
        // Week
          ...
          // Day
              ...
              td.setAttribute('id', month + '_' + year + '_day_' + countDay);
...
function addCalendarEvent(month, year, date, tag, description) {
  var firstDay = zeller(month, year);
